...
Fonts.general.args.fontSlug = ACH:Toggle(L["Font Slug"], L["Appends the SLUG rendering flag for sharper edges."], 4, nil, nil, nil, nil, nil, not E.Retail)
...
Cosmetic.customGlowGroup.args.style = ACH:Select(L["Style"], nil, 1, function() local tbl = {} for _, name in next, E.Libs.CustomGlow.glowList do tbl[name] = L[name] end return tbl end)
...
blizz.itemLevelInfo = ACH:Group(L["Item Level"], nil, 40, nil, function(info) return E.db.general.itemLevel[info[#info]] end, function(info, value) E.db.general.itemLevel[info[#info]] = value M:ToggleItemLevelInfo(nil, true) end, nil, not (E.Retail or E.Mists or E.Wrath))
